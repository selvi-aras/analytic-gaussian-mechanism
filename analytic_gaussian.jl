...
function analytic_noise(epsilon::Float64, delta::Float64, sensitivity::Float64)
    if epsilon < 0 || delta <= 0 || sensitivity < 0
        error("Revise the privacy parameters.")
    end
    mu_Gauss = 0.0
    sigma_Gauss = calibrateAnalyticGaussianMechanism(epsilon, delta, sensitivity)
    return rand(Normal(mu_Gauss, sigma_Gauss), 1)[1] #sample from normal dist
end
